 ...
 ***********************************************************************
 *
 *     Jan 31, 2013 - Added support for C55xx boards
 *
*/
...
int main(int argc, char * argv [])
{
  ...
  else /* if c55xx */
  {
	  if(fileName != NULL)
	  {
		  ...
		  exitrc = serial_open(port, baudrate, &fd);
		  if(exitrc != 0)
		  {
	          fprintf(stderr, "\n%s open failed\n", port);
...
//#endif
		  ...
		  buf[0] = '2';
		  buf[1] = '9';
		  exitrc = serial_send(&fd, buf, 2); /* Sending DSP program command  */
		  if(exitrc != 0)
		  {
		      fprintf(stderr, "\nSending DSP program command failed\n");
...
#if 1
		  // Increase the baudrate for data transfer
		  baudrate = 115200;
		  //serial_close(&fd);
		  ...
		  //fprintf(stderr, "\nReceived ACK for program commnad\n");

		  if (strcmp(buf, "OK") == 0)
		  {
			  fprintf(stderr, "\nTarget connected. Sending data...\n");
	  ...
	  {
	      fprintf(stderr, "\nFilename missing. Use '-f <Filename>' option.\n");
	      exitrc = -1;
	  }
  }
